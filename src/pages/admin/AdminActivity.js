...
export const logActivity = async (type, details, userId = '', userEmail = '') => {
    try {
        await push(ref(database, 'activityLogs'), { type, details, userId, userEmail, timestamp: Date.now() });
...
const AdminActivity = () => {
    ...
    useEffect(() => {
        const unsub = onValue(
            ...
            (error) => { console.warn('activityLogs read error:', error.message); setLogs([]); setLoading(false); }
        );
        return () => unsub();
    }, []);
    ...
    return (
        <Box sx={{ p: { xs: 2, sm: 3, md: 4 }, maxWidth: 1200, mx: 'auto' }}>
            <Box sx={{ mb: 3 }}>
                <Typography variant="h4" sx={{ fontWeight: 800, color: txtMain, mb: 0.5 }}>Activity Logs</Typography>
                <Typography variant="body2" sx={{ color: txtSub }}>Monitor system events and user activity</Typography>
            </Box>
            ...
            <Tabs value={tab} onChange={(e, v) => setTab(v)} variant="scrollable" sx={{ mb: 3, '& .MuiTab-root': { color: txtSub, textTransform: 'none', fontWeight: 600, minWidth: 'auto' }, '& .Mui-selected': { color: '#EE791A' }, '& .MuiTabs-indicator': { backgroundColor: '#EE791A' } }}>
                    ...
                    <Table size="small">
                        ...
                        <TableBody>
                            {filtered.slice(0, 100).map(log => {
                                ...
                                return (
                                    <TableRow key={log.id} sx={{ '& td': { color: txtSub, borderColor: divider, py: 1.2 } }}>
                                        <TableCell><Typography variant="caption" sx={{ color: txtDim }}>{log.timestamp ? new Date(log.timestamp).toLocaleString() : '—'}</Typography></TableCell>
                                        <TableCell><Chip icon={ac.icon} label={ac.label} size="small" sx={{ bgcolor: `${ac.color}15`, color: ac.color, fontWeight: 600, fontSize: '0.7rem', height: 24, '& .MuiChip-icon': { color: ac.color } }} /></TableCell>
                                        <TableCell><Typography variant="body2" sx={{ fontSize: '0.82rem', color: txtSub }}>{log.details || '—'}</Typography></TableCell>
                                        <TableCell><Typography variant="body2" sx={{ color: txtDim, fontSize: '0.8rem' }}>{log.userEmail || '—'}</Typography></TableCell>
                                    </TableRow>
                                );
                            })}
                            {filtered.length === 0 && (
                                <TableRow>
                                    <TableCell colSpan={4} sx={{ textAlign: 'center', color: txtDim, py: 6 }}>
                                        <TimelineIcon sx={{ fontSize: 48, color: txtDim, mb: 1, display: 'block', mx: 'auto' }} />
